...
int main(int argc, char* argv[])
{
	...
	/*
	if (argc < 4) {
		cout << "Mancano dei parametri mettere -f percorso sequenza -s spacedseed" << endl; 
		return 0;
	}
	*/
	...
	//string spacedSeed = "10110111010110101010100101011101001";

	//caloclare le prestazioni 
	time_t time0;   
	time_t time1;
	//prova 
	...
	cout << "Tempo metodo stupido " << seconds << '\n';

	time(&time0);
	vector<vector<uint64_t>> prova_3 = sequenzeTotaliSeedUni(spacedSeed);
	time(&time1);
	seconds = time1 - time0;
	cout << "Tempo metodo tutti uni " << seconds << '\n';


	time(&time0);
	vector<vector<uint64_t>> prova_2 = finalHash(prova_3, metodoNuovo_interaSeq(spacedSeed));
	time(&time1);
	seconds = time1 - time0;
	cout << "Tempo metodo nostro " << seconds << '\n';
	//vector<vector<uint64_t>> prova_3 = sequenzeTotaliSeedUni(spacedSeed);

	/*
	for (int i = 0; i < prova_1.size(); i++) {
		for (int j = 0; j < prova_1.at(i).size(); j++) {
			if (prova_1.at(i).at(j) == prova_2.at(i).at(j))
				cout << "giusto" << endl;
		}
	}
	*/
	...
	for (int i = 0; i < prova_1.size(); i++) {
		...
		for (int j = 0; j < prova_1.at(i).size(); j++) {
			cout << "Hash stupido: " << prova_1.at(i).at(j) << "  Hash nostro: " << prova_2.at(i).at(j) << "Hash tutti uni: " <<  prova_3.at(i).at(j)<< endl;
		}
	}
	*/
	cout << "fine main" << endl;
...
vector <uint64_t> total_seq_hash(string sequenza_input, int spaceSeedSize) {
	...
	for (int i = 1; i <= sequenza_input.length() - spaceSeedSize; i++)
	{
		...
		hashVector.push_back(restanti_Hash(hashVector[i - 1], sequenza_input[i-1], sequenza_input[spaceSeedSize + i - 1], spaceSeedSize));
	}
...
uint64_t metodoNuovo_restantiHash(string past, string token, vector<int> ab, uint64_t hashVecchio, vector<int> comandi) {
	...
	//rotate a sx di a-b 
	uint64_t newHash = leftRotate(hashVecchio, ab[0]-ab[1]);
	...
	for (int i = 0; i < comandi.size(); i++) {
		switch (comandi[i])
		{
			//inserimento
		case 1:
			newHash = newHash ^ (leftRotate(toInt(token[(i+ab[1])%(comandi.size())]), (comandi.size() - (i + ab[1]) % (comandi.size()) - 1)));
			break;
			//cancellazione
		case 2:
			newHash = newHash ^ (leftRotate(toInt(past[(i + ab[0]) % (comandi.size())]), ((ab[0]-ab[1])+comandi.size() - (i + ab[0]) % (comandi.size()) - 1)));
			break;
			//entrambe
		case 3:
			newHash = newHash ^ (leftRotate(toInt(token[(i + ab[1]) % (comandi.size())]), (comandi.size() - (i + ab[1]) % (comandi.size()) - 1)));
			newHash = newHash ^ (leftRotate(toInt(past[(i + ab[1]) % (comandi.size())]), ((ab[0] - ab[1])+comandi.size() - (i + ab[0]) % (comandi.size()) - 1)));
			break;
